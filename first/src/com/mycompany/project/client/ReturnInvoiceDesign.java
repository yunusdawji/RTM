 ...
 */
public class ReturnInvoiceDesign {
	private ReturnInvoiceData data = new ReturnInvoiceData();
	...
	//		.setHorizontalAlignment(HorizontalAlignment.RIGHT);
		...
		TextColumnBuilder<String> descriptionColumn = col.column("Description", "description", type.stringType())
			.setFixedWidth(250);
		TextColumnBuilder<String> tagnoColumn = col.column("Tag No", "tagno", type.stringType())
				.setFixedWidth(50);
		...
		//vat = price * tax
		TextColumnBuilder<BigDecimal> discountColumn = col.column("Discount", "discount", Templates.currencyType);
		//total = price + vat
		TextColumnBuilder<BigDecimal> totalColumn = priceColumn.subtract(discountColumn)
		...
		report
			...
			//columns
			.columns(
				rowNumberColumn, descriptionColumn, tagnoColumn, quantityColumn, unitPriceColumn, totalColumn, priceColumn, taxColumn, discountColumn)
			.columnGrid(
				rowNumberColumn, descriptionColumn, tagnoColumn, quantityColumn, unitPriceColumn,
				grid.horizontalColumnGridList()
					.add(totalColumn).newRow()
					.add(priceColumn, discountColumn))
			//subtotals
			.subtotalsAtSummary(
				totalSum, sbt.sum(priceColumn), sbt.sum(discountColumn))
			//band components
			.title(
				
				Templates.createTitleComponent("Invoice No.: " + data.getInvoice().getId() + "\n" + "Return Date.: " + datecheck.getdate(date)),
				//Templates.createTitleComponent(),
				cmp.horizontalList().setStyle(stl.style(10)).add(
